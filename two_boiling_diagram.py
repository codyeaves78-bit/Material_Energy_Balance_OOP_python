...
def _collect_streams(tb):
    ...
    rows = [
        ...
        (18, "C Final Molasses",        tb._final_molasses_out.flow_lb_per_hr,
                                        tb._final_molasses_out.brix,
                                        tb._final_molasses_out.purity),
        (19, "C Molasses Top-off to C Pans", tb._c_mol_top_off.flow_lb_per_hr,
...
def plot_two_boiling(tb, show: bool = True, save_path: str = None,
    ...
    # 19 C molasses top-off — taps the same point the final molasses leaves
    # from, but heads up and over (clear of the C-magma footing loop at
    # x=24.0) before dropping into C pan's right side.
    dot(PX['C'] + 0.85, 4.85, MOLC)
    seg([(PX['C'] + 0.85, 4.85), (PX['C'] + 0.85, 9.5), (20.3, 9.5), (20.3, 11.0)], MOLC)
    arr(20.3, 11.0, PX['C'] + 1.2, 11.0, MOLC)
    tag(PX['C'] + 0.85, 7.0, 19, MOLC)

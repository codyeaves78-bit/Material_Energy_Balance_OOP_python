...
class ThreeBoiling:
    """An object to do the Three Boiling Balance (single magma stage).

    This is a variant of ThreeBoilingDoubleMagma with the B magma stage
    removed: B centrifugal sugar is a terminal product (sent straight to
    the warehouse alongside A sugar) instead of being remelted into a
    magma that foots the A pans. Only C magma exists, and it is split
    three ways instead of two: a percentage foots the A pans, a percentage
    foots the B pans, and the remainder goes to remelt (back into the
    syrup feed) — the same "percentage of the total, remainder implied"
    pattern FourBoilingDoubleMagma uses for splitting B magma between its
    A1 and A2 pans.
    ...
    def __init__(
            ...
            c_magma_A_footing_pct: float = 40,
            c_magma_B_footing_pct: float = 40, # remainder (100 - A footing - B footing) goes to remelt
        ...
        self.c_magma_A_footing_pct = c_magma_A_footing_pct
        self.c_magma_B_footing_pct = c_magma_B_footing_pct
        self.c_magma_remelt_pct = 100.0 - c_magma_A_footing_pct - c_magma_B_footing_pct
    ...
    def _solve(self, iterations: int = 20):
        ...
        # Both are overwritten each iteration from the same C centrifugal sugar stream (via
        # make_magma), split by c_magma_A_footing_pct / c_magma_B_footing_pct. Brix/purity/temp
        # are placeholders only; the loop replaces them before they matter.
        c_magma_A_pans = SugarStream(brix=self.c_magma_brix, purity=85, flow_lb_per_hr=0, temp_deg_F=130)
        ...
        for _ in range(iterations):
            ...
            # B sugar is a terminal product here (no B magma stage) — it never gets remade
            # into a footing magma, so it plays no further part in the recycle loop.
            ...
            c_magma_A_pans.flow_lb_per_hr = self.c_magma_A_footing_pct / 100 * c_magma_A_pans.flow_lb_per_hr

            c_magma_B_pans = SugarStream.copy(c_magma)
            c_magma_B_pans.flow_lb_per_hr = self.c_magma_B_footing_pct / 100 * c_magma_B_pans.flow_lb_per_hr
    ...
    def total_raw_sugar(self) -> SugarStream:
        """Object of combined Sugars leaving process — A sugar + B sugar. Both are terminal
        products here (there is no B magma stage to remelt B sugar back into the floor)."""
        a = self.A_centrifugals.sugar_stream
        b = self.B_centrifugals.sugar_stream
        total_flow = a.flow_lb_per_hr + b.flow_lb_per_hr
        total_solids = a.solids_flow + b.solids_flow
        total_pol = a.pol_flow + b.pol_flow
        combined = SugarStream.copy(a)
    ...
    def total_water(self) -> SugarStream:
        """All fresh water added to the pan floor (lb/hr): centrifugal wash + C magma
        mingler + C remelt + molasses dilution. No B magma mingler/remelt water here —
        there is no B magma stage."""
        ...
        total_lb_hr  = (cen_wash + c_mingler + c_rmlt_water + a_dil_water + b_dil_water
                        + c_top_off_dil_water)
    ...
    def generate_pfd(self, show=True, save_path=None, include_table=True):
        ...
        from three_boiling_single_magma_diagram import plot_three_boiling
    ...
    def to_excel(self, workbook):
        ...
        from three_boiling_single_magma_diagram import _collect_streams, _collect_water
        ...
        b_sugar = self.B_centrifugals.sugar_stream
        ...
        combined_sugar = self.total_raw_sugar
        pol_extr = combined_sugar.pol_flow / self.syrup.pol_flow * 100

        sw = SheetWriter(workbook, "Pan Floor - Three Boiling", ncols=9)
        sw.title("Three Boiling (Single Magma) — Pan Floor",
                 f"Syrup {self.syrup.flow_lb_per_hr:,.0f} lb/hr @ {self.syrup.brix:.1f} Bx "
                 f"| Pol recovered in raw sugar (A+B) = {pol_extr:.2f}%")
        ...
        out_f = a_sugar.flow_lb_per_hr + b_sugar.flow_lb_per_hr + c_mol.flow_lb_per_hr + total_evap
        out_s = a_sugar.solids_flow + b_sugar.solids_flow + c_mol.solids_flow
        out_p = a_sugar.pol_flow + b_sugar.pol_flow + c_mol.pol_flow
        sw.table(HDRS, [
            ...
            srow("B Product Sugar", "Out", b_sugar),
        ...
        sw.row("Pol % recovered in raw sugar (A + B sugar / feed)", pol_extr, "%", col=4)
        ...
        pan_table(sw, self.A_pans, ["Syrup", "C Magma (A Footing)", "A Molasses Top-off"])
        ...
        pan_table(sw, self.B_pans, ["C Magma (B Footing)", "A Molasses", "B Molasses Top-off"])
        sw.section(f"B CENTRIFUGALS  [{self.B_centrifugals.name}]  "
                   "(sugar is a terminal product — no B magma stage)")
    ...
    def properties(self):
        ...
        return {
            ...
            'b_sugar_flow_lb_hr':             b_sugar.flow_lb_per_hr,
            'b_sugar_brix':                   b_sugar.brix,
            'b_sugar_purity':                 b_sugar.purity,
            'combined_raw_sugar_flow_lb_hr':  combined_sugar.flow_lb_per_hr,
            'combined_raw_sugar_brix':        combined_sugar.brix,
            'combined_raw_sugar_purity':      combined_sugar.purity,
    ...
    def neat_display(self):
        ...
        out = []
        out.append(HEAVY)
        out.append(f"{'THREE BOILING - COMPLETE FLOOR BALANCE':^{W}}")
        out.append(HEAVY)

        # ── Overall ──────────────────────────────────────────────────────
        out.append(_section("OVERALL FLOOR BALANCE"))
        out.append(f"  (Feed = Evaporator syrup + Wash Water; Products = A sugar + B sugar + C final molasses)")
        out.append("")
        out.append(_hdr())
        out.append("")
        out.append("  ENTERING")
        ...
        out.append(_stream("  B Product Sugar", b_sugar))
        ...
        out_flow   = a_sugar.flow_lb_per_hr + b_sugar.flow_lb_per_hr + c_mol.flow_lb_per_hr + total_evap
        out_solids = a_sugar.solids_flow    + b_sugar.solids_flow    + c_mol.solids_flow
        out_pol    = a_sugar.pol_flow       + b_sugar.pol_flow       + c_mol.pol_flow
        out_water  = ((a_sugar.flow_lb_per_hr - a_sugar.solids_flow)
                    + (b_sugar.flow_lb_per_hr - b_sugar.solids_flow)
        ...
        out.append(f"  {'Pol% Recovered in Raw Sugar (A+B sugar / feed):':<{LBL+NUM}} {pol_extr:6.2f} %")
        out.append("")
        ...
        # ── B Centrifugals ─────────────────────────────────────────────────
        out.append(_section(f"B CENTRIFUGALS  [{self.B_centrifugals.name}]  (sugar is a terminal product — no B magma stage)"))
        out.append(_hdr())
        out.append("")
        ...
        # ── C Magma Distribution (informational — feeds A + B pan footing, remainder to remelt) ──
        out.append(_section(f"C MAGMA DISTRIBUTION  (mingler target {self.c_magma_brix:.1f} Bx, "
                            f"remelt target {self.c_remelt_brix:.1f} Bx)"))
        out.append(_hdr())
        out.append("")
        out.append(_stream("  C Magma (total)", self._c_magma))
        out.append(_stream(f"  -> A Pans Footing ({self.c_magma_A_footing_pct:.0f}%)", self._c_magma_A_pans))
        out.append(_stream(f"  -> B Pans Footing ({self.c_magma_B_footing_pct:.0f}%)", self._c_magma_B_pans))
        out.append(_stream(f"  -> Remelt ({self.c_magma_remelt_pct:.0f}%)", self._c_magma_to_rmlt))
        out.append(_stream("  C Remelt (diluted, back to syrup)", self._c_remelt))
...
if __name__ == "__main__":
    pan_floor = ThreeBoiling(
        ...
        C_reheaters=Reheater(massecuite_in=None, massecuite_flow_lb_hr=0,
                             ...
                             name="C Reheaters"),
        c_magma_A_footing_pct=40,
        c_magma_B_footing_pct=40,

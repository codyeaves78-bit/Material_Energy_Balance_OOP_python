...
def _collect_streams(tb):
    ...
    rows = [
        ...
        (25, "C Final Molasses",        tb._final_molasses_out.flow_lb_per_hr,
                                        tb._final_molasses_out.brix,
                                        tb._final_molasses_out.purity),
        (26, "B Molasses Top-off to B Pans", split(bm, tb.b_mol_top_off_pct), bm.brix, bm.purity),
        (27, "C Molasses Top-off to C Pans", tb._c_mol_top_off.flow_lb_per_hr,
...
def plot_three_boiling(tb, show: bool = True, save_path: str = None,
    ...
    # 26 B molasses top-off — taps the trunk right where it turns off the
    # centrifugal and runs back up into B pan's side (mirrors 7/8's A
    # top-off pattern; retraces part of the trunk's own down-leg like
    ...
    # 27 C molasses top-off — taps the same point the final molasses leaves
    # from and loops back up into C pan's right side, clear of the C-magma
    # footing loop (tag 23), which runs further out at x=22.8.
    dot(PX['C'] + 0.85, 4.85, MOLC)
    seg([(PX['C'] + 0.85, 4.85), (21.6, 4.85), (21.6, 11.0)], MOLC)
    arr(21.6, 11.0, PX['C'] + 1.2, 11.0, MOLC)
    tag(21.6, 7.9, 27, MOLC)

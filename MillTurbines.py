# MillTurbines: solves a set of mill drive turbines from the input lists
# and a fiber rate, then reports them side by side in one table.
...
class MillTurbines:
    """
    Solves every mill drive turbine and holds the results.

    Inputs:
        hp_ton_fiber_hr       : list of HP per ton fiber per hour demand,
                                one entry per mill (Mill 1, Mill 2, ... Last Mill)
    ...
    def __init__(self, hp_ton_fiber_hr, isentropic_efficiency, live_steam_object, exhaust_psia, tons_fiber_hr):
        ...
        # map the inputs into the mill turbine dictionary
        self.mill_turbines = {
        ...
        # solve each turbine
        self.turbines = []
        for i in range(len(self.mill_turbines['hp_list'])):
            trb = Turbine(
                inlet_steam=self.mill_turbines['live_steam_object'],
                outlet_pressure_psia=self.mill_turbines['exhaust_psia'],
                isentropic_efficiency=self.mill_turbines['isentropic_efficiency'][i] / 100,  # get % to decimal form
                hp_demand=self.mill_turbines['hp_list'][i],
                name=f"Mill {i + 1}",
            )
    ...
    def __repr__(self):
        return (
            f"MillTurbines({len(self.turbines)} mills, "
    ...
    def display_mill_turbine_information(self):
        def fmt_x(x):
            return "Superheat" if x is None or x >= 1.0 else f"{x:.4f}"

        C0, C1, C2, C3, C4, C5, C6, C7, C8, C9, C10 = 8, 12, 13, 9, 8, 10, 8, 8, 8, 8, 9
        ...
        hdr2 = (f"{'Mill':^{C0}} | {'lb/hr':^{C1}} | {'lb/hr':^{C2}} | "
                f"{'':^{C3}} | {'':^{C4}} | {'lb/HP-hr':^{C5}} | "
                f"{'psia':^{C6}} | {'temp °F':^{C7}} | "
                f"{'psia':^{C8}} | {'temp °F':^{C9}} | {'quality':^{C10}}")

        print(div)
        print(f"MILL TURBINES  —  {self.tons_fiber_hr:,.0f} TON FIBER/HR".center(W))
        print(div)
        print(hdr1)
        print(hdr2)
        print(sep)
        ...
        for i, trb in enumerate(self.turbines):
            ...
            print(f"{trb.name:>{C0}} | {trb.steam_flow_lb_hr:>{C1},.0f} | {trb.exhaust_available:>{C2},.0f} | "
                  f"{trb.hp_demand:>{C3},.0f} | {self.mill_turbines['hp_ton_fiber_hr'][i]:>{C4},.1f} | "
        ...
        print(f"{'TOTAL':>{C0}} | {self.total_inlet_flow_lb_hr:>{C1},.0f} | {self.total_exhaust_available_lb_hr:>{C2},.0f} | "
              f"{self.total_hp:>{C3},.0f} | {sum(self.mill_turbines['hp_ton_fiber_hr']):>{C4},.1f} | "
...
if __name__ == "__main__":
    mills = MillTurbines(
        hp_ton_fiber_hr=[20, 18, 18, 18, 18, 20],        # Mill 1, Mill 2, ... Last Mill
    ...
    print(mills)
    print()
    mills.display_mill_turbine_information()
